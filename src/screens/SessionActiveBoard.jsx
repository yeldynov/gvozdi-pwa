  ...
  return (
    <div className="h-full flex flex-col bg-bg pt-[52px] px-5 pb-7 animate-fade-up-md">
      ...
      <div className="flex justify-between items-center mb-[14px]">
        <button onClick={() => nav('home')} className="border-none bg-transparent text-text-2 flex p-0">
          <Icons.close size={22} />
        </button>
      ...
      <div className="flex items-baseline justify-between mb-4">
        <div>
          <div className="num display text-[48px] leading-none font-extralight">{fmt(t.remaining)}</div>
          <div className="text-text-3 mt-[2px] uppercase text-[11px] tracking-[0.1em]">
            remaining · {Math.round(t.progress * 100)}% done
          </div>
        </div>
        <div className="text-right">
          <div className="num text-error font-medium text-[22px]">68 <span className="text-text-3 text-[11px] font-normal">bpm</span></div>
          <div className="text-text-3 mt-[2px] text-[11px] tracking-[0.08em]">↘ -4 vs start</div>
        </div>
      </div>

      {/* board */}
      <div className="flex-1 relative flex items-center justify-center bg-bg-2 rounded-lg mb-[14px]" style={{ padding: 18 }}>
      ...
      <div className="grid grid-cols-3 gap-[6px] mb-4">
        {[
          ...
          <div key={s.l} className="card" style={{ padding: 12 }}>
            <div className="text-text-3 uppercase text-[10px] tracking-[0.08em]">{s.l}</div>
